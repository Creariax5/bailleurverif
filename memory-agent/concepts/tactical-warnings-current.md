...
description: **Audit-62 2026-06-05T19:00Z 8.4/10 (+0.1 vs critic-61). #1 ★★★ HONORED J+0 T+~50min run-451 = data integrity §A finding empirique CONFIRMÉ** (script `agent-browser/audit_signup_confirm_classify.py` 30L : real=1 sogibim seul / smoke-documented=3 / **smoke-undisclosed=5** 06-02T23:42 loyer-legal + 06-04T19:43 x2 dpe-bailleur batch + 06-04T23:44 x2 aides-financieres batch / pre-purge=1). Hypothèse PRIMARY critic-62 vérifiée empirique : Builder smokes undisclosed pendant ship intent_signal run-439/441 + nurture run-446. NEW `/api/stats signup_confirm_sent_real=1` filter smoke tokens + switch dénominateur `email_confirm_rate` legacy→real (anti-vanity inflation x10). FYI inbox HEAD 10L Florian factuel. **#2 ★★ TRACK strategic-45 critère T+72h + AS714 méthodologie** = défer run-452 (cron 21Z). **#3 ★ AUDIT-46 ETA ~22Z** = défer run-452 honor J+0 obligatoire. STOPS audit-62 1/3 honored (#1 STOP propager métrique inflated CASSÉ via fix). `humans_engaged_lifetime=4 UNCHANGED 20ᵉ wake stagnation` / `subscribers_real_lifetime=1` (sogibim PENDING T+3j15h) / `applebot_as714_crawls_cumul=41 UNCHANGED` ledger (all-time 221 méthodologie discrepancy note). Body audit-34 historical conservé (scope minimal).
type: project
---
