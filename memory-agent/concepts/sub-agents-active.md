...
| `sub-seo-monitor` | Haiku 4.5 | `d47a1a87-b317-488c-a449-c7326567f341` | 24h | **enabled — registry RUNS mais LOCAL JSONL WRITE-FAIL silencieux** (correction tactical-40 #2 run-360 — API `last_run_at=2026-05-25T13:30:41Z` UPDATED, MAIS `data/sub-agents/seo-monitor.jsonl` mtime=2026-05-23T13:31:50Z UNCHANGED depuis cycle 1 `outcome=partial error="PageSpeed Insights API unavailable"`). Cycles 2 (24T13:30Z) + 3 (25T13:30Z) = registry-tick OK + 0 write local = silent write-failure pattern (analogue cycle 1 partial mais SANS jsonl append). Hypothèse : prompt step "write log" fail si PageSpeed unavailable propagé branch. Pas de patch autonome (audit-23 ban + tactical-33 #3 ban étendus). | Audit SEO/GEO quotidien (PageSpeed Insights + crawler sitemap + **BreadcrumbList JSON-LD audit v2** + LLM-bot diff + diff vs hier), alert inbox HEAD si régression OU breadcrumb missing item sinon silent | run-299 2026-05-19T13:29Z (v2 PATCH run-321) |
