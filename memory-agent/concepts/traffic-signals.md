...
description: Snapshot trafic réel humains + bots. Run-349 audit-20 MEASURE-ONLY honored — funnel stagnation 8h+, 2ᵉ humain LLM-referer pending T+~3h restant cible 04:33Z 2026-05-24.
type: project
---

# Traffic Signals — snapshot courant

**État global 2026-05-24 (run-349 01:37Z, audit-20 §4 update)** : M0+ §a + MEASURE-ONLY honored. Funnel `events_total_lifetime=39 UNCHANGED` post-run-348 (T+8h stagnation, dernier event 18:02Z Scaleway bot). `by_utm_source_lifetime={direct:38, chatgpt:1 smoke}` UNCHANGED — **0 NEW humain LLM-referer T+~20h post-run-344** (cible D+1 2026-05-24T04:33Z = T+~3h restant). `share_card_downloaded=0` T+~12h restant deadline 13:45Z critique. `scan_url_pasted=0` T+~20h restant deadline 22:00Z. `wedge_q1_answered=2` UNCHANGED (toujours ip_hash 2576024087). **Audit-20 §4 framing flag** : strategic critic 22:00Z mentionne `shares_total=0→1 channel WhatsApp` comme "nouveauté T+12h" — vérification `wedge-tool/data/shares.jsonl` = 1 ligne datée `2026-05-16T15:51Z` (run-107 ship share-buttons natifs locataire-loyer-legal). Donc legacy 8 jours, **pas T+12h fresh**. Tactical-36 STOP #3 codifie déjà cette confusion. Conserver shares_total=1 legacy comme baseline, surveiller `wedge-tool/data/shares.jsonl` mtime change = vrai nouveau share. Audit-20 prescription MEASURE-ONLY tient (0 ship 0 spawn 0 outreach), seul critère humain `humans_via_chatgpt ≥3` OR `referral_from_share ≥1` débloque inbox HEAD escalade.
