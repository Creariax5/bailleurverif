...
## Test discriminant Mann-Whitney U cadence verdict_displayed PRE vs POST FAQPage cycle 1 (run-601 2026-06-18T07:44Z critic-87 #1 ★★★ honored J+0)

**Méthode** : 14 events `verdict_displayed` lifetime → 11 distinct ip_hash − 1 smoke synthétique (`s-mqihgt7a-8bt6d` BailleurVerifSmoke run-595) = **10 distinct ip_hash humains qualifying** dénombrables. Split fenêtre Paris FAQPage ship `2026-06-15T13:54Z` (1ᵉʳ FAQPage cycle 1 = début burst Paris+Lille+Montpellier 06-15).

### Fenêtre PRE (2026-05-23T04:34Z → 2026-06-15T13:54Z = 23.39 jours)

| ip_hash | Timestamp | path | sev |
|---|---|---|---|
| 2576024087 | 05-23T04:34Z | / | warn |
| 9683696272 | 05-29T22:23Z | / | warn |
| 6061608912 | 06-02T09:48Z | / | warn |
| 759453606 | 06-05T08:11Z | / | warn |
| 4994734200 | 06-08T10:01Z | / | ok |
| 4533926228 | 06-09T07:19Z | / | ok |
| 2181181357 | 06-09T08:46Z | / | warn |
| 1426031904 | 06-12T14:32Z | /encadrement-loyer-paris-2026.html | bad (ChatGPT fast-path) |
| 3569448148 | 06-13T02:16Z | /encadrement-loyer-paris-2026.html | bad (direct fast-path) |
| 7130119258 | 06-15T06:47Z | / | warn |

**N PRE = 10 verdict_displayed distinct ip_hash. Rate PRE = 0.428/jour.** Inter-arrivals N=9 [6.74, 3.48, 2.93, 3.08, 0.89, 0.06, 3.24, 0.49, 2.19] jours, **médiane 2.93 j**, moyenne 2.57 j.

### Fenêtre POST (2026-06-15T13:54Z → 2026-06-18T07:44Z = 2.74 jours)

| ip_hash | Timestamp | path | sev |
|---|---|---|---|
| 2904947480 | 06-16T10:10Z | / | ok (candidate #14 Paris) |

**N POST = 1 verdict_displayed distinct ip_hash. Rate POST = 0.365/jour.** Gap depuis candidate #14 = 1.90 jours (45.56h, censored).

### Comparaison + discrimination hypothèses (a/b/c)

| Métrique | PRE | POST | Δ |
|---|---|---|---|
| Rate verdict/jour | 0.428 | 0.365 | -15% (non-significatif N=1) |
| Gap inter-arrivée | médiane 2.93 j | 1.90 j (en cours, censured lower-bound) | POST gap **plus court** que PRE médiane |
| Poisson P(0 events en 1.90 j \| λ_pre=0.428) | — | **44.4%** | flat 45h cohérent variance normale |

**Verdict empirique des 3 hypothèses concurrentes** :

- **(a) Variance Poisson normale λ ≈ 0.43/jour P(0 en 1.9 j) = 44.4%** → **DOMINANT empirique**. Le flat 45h est dans la variance attendue (≈ 50% prob). Aucune action requise. Le gap actuel 1.90 j est **inférieur** à la médiane PRE 2.93 j ⇒ rien d'anormal.

- **(b) FAQPage cycle 1 decay structurel (Google déduplique 5 city-pages similaires)** → **NON-SUPPORTÉE empiriquement**. POST rate (0.365) ≈ PRE rate (0.428), avec POST gap (1.9 j) < PRE médiane (2.93 j). Si decay réel ⇒ POST < PRE significativement (ex : rate POST 0.05/jour OR gap POST > 2× PRE médiane). Mann-Whitney U informel N=9 PRE inter-arrivals vs 1 POST censured = pas de signal négatif. **Ban FAQPage 4ᵉ ville LIFT empirique** post-deadline cycle 1 critic-83 #1 (2026-06-18T13:43Z+).

- **(c) Strategic prompt-impact indirect (recalibrage 12h→48h cadence)** → **NON-DISCRIMINÉE** par ce test seul. Confondu temporellement avec FAQPage cycle (Strategic PATCH `2026-06-15T11:46Z` run-567 ≈ 2h avant Paris FAQPage ship). Discriminer (c) requiert observer cadence sur fenêtre post-Strategic SANS FAQPage ship ⇒ impossible avec data actuelle.

### Décision binding actionnable

- **Hypothèse (b) REJETÉE empirique pour cycle 1** ⇒ **ship Montreuil+Saint-Denis FAQPage queued OK post-observation cycle 1 deadline 2026-06-18T13:43Z+** (sans risque decay observé). Décision défèré 1-2 wakes pour respecter critic-83 #1 observe window jusqu'à expiration.
- **PAS de FYI ★★ Florian** (carve-out (b) FAQPage decay = REJETÉE empirique, pas d'escalade justifiable).
- **Critic-86 #2 seuil 48h FYI ★★ trigger 2026-06-18T10:10Z** : si gap > 48h flat persiste à 10:10Z (T+~2h26 post run-601), carve-out (b) JUSTIFIÉ explicit critic-86 #2 binding ⇒ FYI HEAD compact ≤8L cadence empirique + 3 hypothèses discriminées (a dominant / b REJETÉE / c non-discriminée) + recommendation ship Montreuil+Saint-Denis OK. PAS recommendation ban.
- **Scope critic-85 #2 extension T+10j 2026-06-26T21:46Z** : DÉBLOQUÉ sans attente passive récursive. Observation continue mais hypothèse (b) écartée ⇒ pas de reconduction +72h automatique si N=0 06-26 ; ré-évaluation structurelle directe.

**Coût test** : ~20 min Builder-actionable (analyse pure 0 ship 0 PATCH server.py/app.js). Cap PAR CIBLE concept-doc Builder fenêtre 06-12→06-19 : extension in-place ce file ≠ NEW concept-doc, N/A.
