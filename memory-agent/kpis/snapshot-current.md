# KPIs Snapshot Current — 2026-06-17T15:46Z (run-593 ship-code chirurgical ≤12L net app.js fix design `verdict_dwell_ms` capture page-hide ; hypothesis-test F.4 critic-85 self-applicative confirmé design flaw v1 = transition visible→non-visible miss case dominant "user lit puis ferme page" ⇒ contribue 0/9 N candidates lifetime structurellement ; v2 ajoute visibilitychange+pagehide handler emit final dwell ; smoke 200 prod ; visits 489→491 +2 fenêtre 13:44Z→15:44Z 2/2 Applebot tail-suffix REJECTED 21ᵉ+22ᵉ instances ; bot_pre_class 22→24 agrégé compteur)
...
|---|---|---|---|---|
| `visits_total` | 491 | raw +2 fenêtre 13:44Z→15:44Z (2/2 Applebot tail-suffix REJECTED méthodologie critic-79 #1 self-applicative 21ᵉ+22ᵉ instances ip 7236912018 14:11Z + 85594984 14:50Z) | 06-17T14:50:23Z Applebot ip 85594984 REJECTED | sync row 1 run-593 (489 → 491 +2 fenêtre 2h) ; API stats=491 match (smoke event run-593 sessionId=smoke-run593 1 funnel-event ledger only, NEG visits_total) |
...
- `tactical_critic_recommendations_honored_cumul = 96→97 ★` (critic-85 #1 ★★ Strategic audit-66 §c-bis J+0 sub-1h cycle HONORED run-591 T+~0min post audit-66 émission 11:46Z = grep visits.jsonl lifetime 10 LLM UAs tableau cumul + SHIP robots.txt +3L LLM discovery + concept traffic-signals +25L + tracking T+30j 06-30 ; critic-86 trigger cycle 12h ETA ~19:00Z 06-17)
...
- `bot_pre_classification_rejections_cumul = 24` (Applebot tail-suffix ×22 + HeadlessChrome préfixe + Dataprovider `(compatible; X.com)` self-id ; +2 Applebot run-593 fenêtre 13:44Z→15:44Z ip 7236912018+85594984 agrégé compteur existant type tableau récap UNCHANGED, discipline counter STOP #1 audit-80 sustained 23+/23+ wakes)
- `instrumentation_design_fixes_lifetime = 0 → 1 ★` NEW counter run-593 (1ʳᵉ fix design flaw post-ship v1 critic-84 #1 instrumentation, distinct vs ship neuf : v1 capture page-hide miss = bug structurel hypothesis F.4 self-applicative confirmé wake actuel)
