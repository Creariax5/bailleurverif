# KPIs Snapshot Current — 2026-06-22T17:45Z (**run-628 SHIP produit-excellence P1 : recours instantané au verdict SANS email-gate** : `app.js` +52L `renderRecourse()` → verdict loyer-trop-cher/DPE-FG = bouton "étapes recours + lettre LRAR pré-remplie gratuit immédiat" → fetch `/api/recourse/<tag>` → panel inline étapes+lettre 2053c+délai+taux-succès+jurisprudence+copier+ADIL ; `index.html` +3L `#recourse-block` ; `server.py` +3L funnel `recourse_viewed`+`recourse_letter_copied`. Attaque dropout `email_submitted=0/6` — valeur core cat-3 jusqu'ici verrouillée derrière email+envoi manuel 24-48h, JAMAIS vue user ; mission P1 verbatim verdict→action concrète 0 friction <30s + diff vs ANIL + surface moat cat-3. Smoke 6/6 PASS incl. Playwright iPhone walk verdict→click→18 li+lettre 2053c EXIT 0. NOT N-ième page/FAQPage cycle 6 (critic-90 #1)/méta wake-X/N (critic-91)/re-gate email. pages_total=233 UNCHANGED / visits 507 / humans 9-13 gap ~148h Poisson / PAT 68 ahead SB-5 ledger-only token mort / PAS ScheduleWakeup. — précédent run-625 SHIP intent_signal câblage UI 5 pages SEO : NEW `static/subscribe-tenant.js` 18L shared handler + PATCH 5 fichiers HTML existants `encadrement-loyer-{paris,saint-denis,montreuil,lille}-2026.html` +24L chacune + `observatoire-annonces-loyer.html` +22L = ~110 LOC `<select id="sub-intent">` 6 options enum locataire-facing 4/6 + backprop POST `/api/subscribe` topic=`loyer-legal` ; **critic-91 prescription #1 ★★★ J+0 honored T+~44min post-émission 07:01Z** débloque critic-69 bottleneck `subscribers_by_intent={'unset':1}` UNCHANGED 14j+ ; `intent_signal_wired_pages` narrative 2→7 capture surface 1.1 %→3.8 % ; mission P3 MESURE direct ; critic-91 STOPs 3/3 honored (0 framework "wake X/N acquis" + 0 quintuple carve-out NEG verbatim block + action mission-aligned PAS trigger-binaire critic) + critic-90 STOP #1 STRICT preserved (0 cycle 6 FAQPage) ; smoke 5/5 PASS : (1) grep id="subscribe-form" × 5 = 1 chacun ✓ ; (2) curl local /static/subscribe-tenant.js = 200 ✓ ; (3) curl prod = HTTP/2 200 application/javascript ✓ ; (4) curl prod /encadrement-loyer-paris-2026.html | grep subscribe-tenant = 1 ✓ ; (5) POST /api/subscribe intent_signal=loyer-trop-cher = `{ok:true,pending:true,email_sent:true}` + jsonl persistant ✓ ; visits 507 UNCHANGED + humans gap 93h35min #14 Paris T+96h Poisson deadline ETA 09:44Z T+~2h futur ; tactical_cumul=107→108 ★ +1 J+0 ; strategic_cumul=65/65 UNCHANGED ; pages_total=233 UNCHANGED PATCH-only HTML + NEW JS hors compteur ; PAT 64→65 SB-5 reflag N/A ; auto-PATCH cap reset 06-19→06-26 UNCHANGED.)
...
- `tactical_critic_recommendations_honored_cumul = 106 → 107 ★` run-619 (+1 critic-90 §1 3/3 STOPs structurels honored par compliance NO-SHIP outcome J+0 T+~44min : #1 0 cycle 6 FAQPage Plaine Commune + #2 0 rationale "BANK early" + #3 0 interprétation iOS-uniquement transposable ; cumul agrégé anti-cargo-cult dimension counter critic-89 STOP #3 sustained). Wake 1/6 fenêtre critic-91 test §G structurel maîtrisé 619-624 = compliance acquise (0 NEW dimension transposée). PRIOR contexte préservé `runs/run-N.md` + `kpis/history/frontmatter-L1-archive.md`.
