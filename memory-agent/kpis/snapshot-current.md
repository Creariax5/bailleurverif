# KPIs Snapshot Current — 2026-06-23T03:45Z (**run-633** PURGE INTÉGRITÉ moat cat-3 P1 : template `loyer-abusif.v0.json` avait 2/3 jurisprudence_refs hors-sujet = décisions Cass civ2 **assurance-vie** (C200810/C200808, analogie forcée run-sub-1) présentées comme jurisprudence d'encadrement locatif dans un actif moat public/servi live au verdict home renderRecourse run-628. Remplacées par 2 décisions Cass **civ3 locatives ECLI+sommaire vérifiés Légifrance** : C300584 (test légal complément de loyer, 04/12/2025) + C300036 (clause indexation non-réciproque nulle art L.112-1 CMF + restitution trop-perçu, Publié au bulletin). 3/3 refs civ3 on-topic. Smoke 6/6 prod /api/recourse/loyer-abusif count=3 toutes civ3 ✓. NOT fabrication ECLI / NOT 4ᵉ fix conv (critic-92 STOP#1) / NOT escalade Florian (SB-5). Incident infra : restart manuel exit 144 ×3 (harness tue enfants shell), résolu watchdog.sh cron, prod 502 ~90s. Archive run-632 ↓.) (**run-632** produit-excellence P1, Florian directive TOP "LRAR pré-remplie" : `depot-garantie-france-2026.html` calculateur → bouton "Pré-remplir le modèle 1 avec X €" injecte montant calculé dans lettre LRAR + scroll/highlight (~16L JS, mirror renderRecourse run-628). Defect réel inspection produit : page calculait montants mais 4 lettres exigeaient remplissage manuel `[…]`. Différenciation interactive vs ANIL statique. NOT 4ᵉ acquisition consécutif post-runs 629/630/631 (anti dimension-hopping critic-79 §G) / NOT 4ᵉ fix conversion (critic-92 STOP #1, délivrance valeur core ≠ email-capture) / NOT smoke filler (STOP #3). Smoke 6/6 PASS. Archive run-630/631 narrative → ledger.md.)
