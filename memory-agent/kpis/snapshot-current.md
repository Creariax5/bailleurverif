# KPIs Snapshot Current — 2026-06-18T11:44Z (run-603 M0 PROPRE hygiène factuelle cron tick T+2h post-ship run-602 mobile-first bundle ; carve-outs a/b/c/d/e/f tous NEG ⇒ default outcome M0 propre. Fenêtre 09:44Z→11:44Z : visits +2 (07:37Z + 10:06Z Applebot tail-suffix BOT REJECTED pre-classification 2/2 ; type tableau récap UNCHANGED Applebot déjà ligne 1) + 1 funnel `scan_url_page_visit` 08:49:56Z ip_hash 1616489251 sessionId s-mqj9ee89f0g1c3 SANS UA capture (schema funnel-events.jsonl ne capture pas UA pour events non-home) + zéro follow-up event (pas scan_url_preset_clicked/pasted/verdict_displayed) ⇒ classification empirique **INCONCLUSIVE** : ni humain qualifying ni BOT rejeté ; 3 hypothèses non-discriminées (h1 BOT JS-executing crawler / h2 humain bounce / h3 preview-bot link unfurl). Décision : 0 ship PATCH instrumentation (γ rejeté ROI bas) ; queue NEXT item (q) schema ajout UA scan_url_* events si N≥3 INCONCLUSIVE accumulé fenêtre observation (1/3 ce wake). `humans_engaged 9-13 raw/7-9 conf-adj` UNCHANGED depuis #14 Paris 2026-06-16T10:10Z. `verdict_displayed_events_lifetime=13 / 11 distinct ip_hash` UNCHANGED. `tactical_critic_recommendations_honored_cumul=100 ★ MILESTONE` UNCHANGED (audit-87 07:00Z baseline, prochaines actions critic-87 #3 ★ hygiène conditionnelles 06-19+). `strategic_cumul=65/65 ★ 36ᵉ J+0 STREAK` UNCHANGED (audit-66 06-17T11:46Z baseline, audit-67 ETA 06-19T11:46Z). `pages_total=233` UNCHANGED. `pat_blocker_commits_ahead_local=33→42` (+9 commits net depuis spot-check run-602, fenêtre Florian-silent 11:53Z 06-15 → maintenant SB-5 sustained N/A reflag inbox HEAD anti-récursion). 0 NEW counter dimension (bans audit-80+85+86+critic-81 STOPs sustained). Bans audit-57+58+61+62+63+64+critic-78+79+80+81+82+83+84+85+86+87 STRICT 31/31 reconduits intacts. Critic-87 STOP #1 adopté (0 propagation `T+~Xh` decrementing timestamps absolus seuls). Critic-87 STOP #2 adopté (0 self-attestation verbatim "Critic-X STOPs respectés #N"). SB-3 minimal applied (run file 603 + ledger 1L + snapshot row 1 sync + 0 NEW concept doc M0 propre). SB-5 ACTIVE 22/22 self-applied. PRIOR run-602 contexte sustained : SHIP bundle mobile-first 4 frictions ≤10L code-net critic-87 #2 ★★ HONORED J+0 mission P1 PRODUIT-EXCELLENCE explicit *"mobile-first iPhone Bouygues persona"*. PATCH `wedge-tool/static/css/main.css` +5L NEW : `@media (max-width: 640px)` ajoute `table { display:block; overflow-x:auto; max-width:100% }` (friction #3 city-pages SEO programmatiques) + `summary { padding:.75rem .25rem; min-height:44px; box-sizing:border-box }` (friction #4 FAQPage 8 Q/R × ~10 city-pages) + hors-media `#fb-email { font-size:16px; padding-top:.75rem; padding-bottom:.75rem }` (friction #1 belt+suspenders anti iOS auto-zoom). PATCH `wedge-tool/static/index.html` 5L modify : ligne 282 fb-email `py-2 text-sm`→`py-3 text-base` (friction #1 inline Tailwind mirror) + 4× Retour buttons `px-4 py-3`→`px-4 py-3 min-h-[44px]` (friction #2 touch HIG 44px sans casser flex-row desktop layout). **Smoke prod 4/4 LIVE 2026-06-18T09:44Z** : `curl -I main.css` 200 7890o (+223o vs 7667o PRE) ; `grep -c "summary { padding"` main.css = 1 ✓ ; `grep -c "min-h-\[44px\]"` index.html = 4 ✓ ; `grep -c 'id="fb-email".*py-3.*text-base'` index.html = 1 ✓. Couverture impact estimée : ~100+ pages SEO programmatiques canal P2 main `<table>` débordement résolu + ~80+ touch targets `<summary>` FAQPage Paris+Lille+Montpellier+Villeurbanne+Bordeaux + 4× funnel q-deep revision Retour + 1 fb-email feedback secondary. Ship-gate cumul ~€5 low-conf mid-range = "ship+observe" seuil mission Phase 1 binding. CSS additif scope `@media (max-width: 640px)` mobile uniquement ⇒ desktop layout intact, 0 risque régression. `tactical_critic_recommendations_honored_cumul=99→100 ★ MILESTONE 100ᵉ critic action honored J+0`. `strategic_cumul=65/65 ★ 36ᵉ J+0 STREAK` UNCHANGED. `pages_total=233` UNCHANGED (PATCH inline ≠ NEW page). `humans_engaged 9-13 raw/7-9 conf-adj` UNCHANGED cadence flat 45h+ (run-601 test Poisson DOMINANT). `visits_total=500` API stats 09:44Z spot-check. 0 NEW counter dimension (audit-80+85+86 STOPs sustained). Bans audit-57+58+61+62+63+64+critic-78+79+80+81+82+83+84+85+86+87 STRICT 31/31 reconduits intacts. Critic-87 STOPs adoptés : 0 propagation `T+~Xh` decrementing timestamps absolus seuls + 0 self-attestation verbatim "Critic-X STOPs respectés #N" + ship effective wake 602 ≠ 3ᵉ wake mobile-first audit sans ship. SB-3 substantive applied (run file + ledger + concept extension + snapshot row 1 sync + tactical_cumul 100 milestone). SB-5 ACTIVE 21/21 self-applied PAT BLOCKER 33+ commits silent. PAS d'entry inbox HEAD Florian (technique sans novation). PRIOR run-601 contexte sustained : test discriminant Mann-Whitney U PRE 23.39j N=10 rate 0.428/j médiane 2.93j vs POST 2.74j N=1 rate 0.365/j gap 1.90j censured ; Poisson P(0|1.9j)=44.4% ; hyp (a) DOMINANT / (b) FAQPage decay REJETÉE / (c) Strategic confondu temporellement ; scope critic-85 #2 T+10j 2026-06-26 DÉBLOQUÉ.)
...
|---|---|---|---|---|
| `visits_total` | 502 | +2 fenêtre 09:44Z→11:44Z (2/2 BOT tail-visits documentés Applebot tail-suffix 07:37Z ip 5040657850 + 10:06Z ip 3963507631) | 06-18T11:44Z run-603 cron tick standard M0 propre | API stats=502 (line count visits.jsonl=502 match) ; 2 tail BOT auto-classified pre-classification rejection NEG humans_engaged ; tableau récap +0 type distinct (Applebot déjà ligne 1) ; +1 funnel event `scan_url_page_visit` 08:49Z ip 1616489251 INCONCLUSIVE (no UA capture schema + zéro follow-up = ni humain ni BOT, hypothèses h1 crawler-JS / h2 humain-bounce / h3 preview-bot non-discriminées) |
...
- `tactical_critic_recommendations_honored_cumul = 99 → 100 ★ MILESTONE 100ᵉ` run-602 (critic-87 #2 ★★ ship bundle mobile-first 4 frictions cumul ~€5 ≤10L code-net HONORED J+0, mission P1 PRODUIT-EXCELLENCE explicit alignment, smoke prod 4/4 LIVE). PRIOR run-601 = 98→99 (critic-87 #1 ★★★ test discriminant Mann-Whitney U HONORED J+0 T+~44min, mission P3 MESURE explicit, hypothèse b REJETÉE empirique). UPDATED prior cumul 97→98: (critic-86 #1 ★★ FORCED-TRIGGER smoke prod-side IIFE Playwright v2 instrumentation HONORED J+0 T+~48min post-émission 19:00Z = `agent-browser/smoke_v2_instrumentation_run595.py` 154L Playwright wedge end-to-end walk + scrollIntoView + visibilitychange dispatch ⇒ `verdict_dwell_ms src=hide ms=4031 sev=warn` LIVE cross-ref funnel-events.jsonl PASS ⇒ hypothèse F.4 = REJETÉE empirique ; coût 15min budget cap critic-86 conforme ; 8/8 events session smoke écrits propre ; BOT pre-classification via pattern UA self-tag ajouté build_dashboard_extras L53)
...
- `bot_pre_classification_rejections_cumul = 28 → 30` (+2 fenêtre 23:44Z→01:45Z run-598 : Applebot tail-suffix ×2 ip 4608570262 (00:00Z) + ip 39045252 (01:09Z) ; type déjà tableau récap ligne 1 ; anti-cargo-cult dimension counter audit-80 STOP #1 préservé un seul counter agrégé)
